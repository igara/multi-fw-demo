...
export function Vue2CounterLoader() {
  ...
  useEffect(() => {
    ...
    const initializeVue2App = async () => {
      try {
          ...
          if (properties.length > 0) {
            ...
            try {
              const win = window as typeof window & {
                __vue2CounterPropsInjected?: boolean;
              };
              if (!win.__vue2CounterPropsInjected) {
                win.__vue2CounterPropsInjected = true;
        ...
        script.onload = () => {
          // window.mountVue2CounterAppが利用可能になるまで待つ
          const checkAndMount = () => {
            if (typeof window.mountVue2CounterApp === "function") {
              try {
                window.mountVue2CounterApp(appContainer);
                setIsLoading(false);
              } catch (err) {
                console.error("Failed to mount Vue2 counter app:", err);
                setError("Vue2カウンターアプリのマウントに失敗しました");
                setIsLoading(false);
              }
            } else {
        ...
        console.error("Failed to initialize Vue2 counter app:", err);
        setError("Vue2カウンターアプリの初期化に失敗しました");
        setIsLoading(false);
      }
    };
